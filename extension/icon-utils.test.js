...
test('getIconSources prefers real favicon before chrome, origin, and Google fallbacks', () => {
  ...
  try {
    ...
    assert.deepEqual(iconData.sources, [
      ...
      'chrome-extension://abc/_favicon/?pageUrl=https%3A%2F%2Fwww.example.com%2Fpage&size=32',
...
test('getGroupIcon falls back through origin and Google sources when tab has no real favicon', () => {
  ...
  assert.deepEqual(iconData.fallbackSources, [
    'https://www.google.com/s2/favicons?domain=chatgpt.com&sz=32',
  ]);
...
test('getIconSources uses chrome, origin, and Google fallback chain when no real favicon is available', () => {
  const savedChrome = globalThis.chrome;
  globalThis.chrome = {
    runtime: {
      getURL: (path) => `chrome-extension://abc/${path}`,
...
test('getIconSources skips unstable browser internal favicon urls but keeps Google fallback URLs', () => {
  const iconData = getIconSources({
...
test('getFaviconUrl preserves http protocol and localhost port in fallback URL', () => {
  const result = getFaviconUrl('http://localhost:3000/app');
  assert.equal(result.fallback, 'http://localhost:3000/favicon.ico');
});

test('getFaviconUrl preserves custom https port in fallback URL', () => {
  const result = getFaviconUrl('https://example.com:8443/path');
  assert.equal(result.fallback, 'https://example.com:8443/favicon.ico');
});
